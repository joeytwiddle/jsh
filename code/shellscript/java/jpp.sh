#!/bin/sh
# jsh-ext-depends-ignore: dir file
# jsh-ext-depends: sed
# jsh-depends: jshinfo verbosely errorexit
# jsh-depends-ignore: before write
...
Usage: jpp [-a] [<macro_files...>] -- <jpp_files_to_compile...>

  will process each supplied file X.jpp file through the GCC C++ pre-processor,
  outputting to stdout, or with -a to the filaname without \.jpp extension.
...
do
	...
	# Output to standard out if we are not using $AUTOOUT
	OUTNAME="-"

	if [ "$AUTOOUT" ]
	then
		## Save to file
		OUTNAME="`echo "$PPNAME" | sed 's+\.jpp$++'`"
		if [ "$OUTNAME" = "$PPNAME" ]
		then . errorexit "Where should I write post-processed $PPNAME ?"
		else jshinfo "Writing to $OUTNAME"
		fi
	fi

	# if [ "$OUTNAME" = "-" ]
	# then jshinfo "Processing $PPNAME to stdout"
	# else jshinfo "Processing $PPNAME to $OUTNAME"
	# fi
	...
	if [ "$PRE_COMMENT" ]
	then echo ; echo "$PRE_COMMENT $comment"
	elif hasExtension "html.jpp"
	...
	# verbosely
	gcc $JPP_GCC_OPTS -x c "$PPNAME" -E $MACRO_ARGS $INCLUDES -o "$OUTNAME"

	# If we don't store it in a var, we lost it immediately after the following test!
	errCode="$?"
	if [ "$errCode" != 0 ]
	then
		# error "[jpp] Errors during pre-processing ($errCode)"
		exit $errCode
	fi
