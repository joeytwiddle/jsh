...
do
	...
	# else
		...
		# # OUTNAME=`echo "$PPNAME" | sed 's+\.jpp$+\.java+'` ## aah this overwrote when it wasn't .jpp!
		# ## Or add ".jpp":
		# OUTNAME=`echo "$PPNAME" | sed 's+$+.jpp+'` ## If it has .jpp it could remove it.  or we could ask as an option
	# fi
	(
