...
then
	...
	then GIT_AWARE_PROMPT="\[`cursenorm`\]\$git_branch\[`cursegreen``cursebold`\]\$git_ahead_mark\$git_ahead_count\[`cursered``cursebold`\]\$git_behind_mark\$git_behind_count\[`curseyellow`\]\$git_dirty\$git_dirty_count\[`cursecyan`\]\$git_staged_mark\$git_staged_count"
	fi
